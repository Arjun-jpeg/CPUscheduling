...
public class FCFSVisualizer extends JFrame {
    ...
    public FCFSVisualizer() {
        setTitle("FCFS CPU Scheduling Visualizer");
        setSize(800, 400); //since it already extends jfram no need of frame.setSize();
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        setLayout(null); // absolute layout for positioning

        JLabel title = new JLabel("FCFS CPU Scheduling Simulation", SwingConstants.CENTER);
        title.setFont(new Font("Arial", Font.BOLD, 20));
        title.setBounds(200, 10, 400, 30); //setBounds should be used when SetLyput is null
        add(title);

        // Ready Queue
        JLabel readyLabel = new JLabel("Ready Queue:");
        readyLabel.setBounds(30, 60, 100, 30);
        add(readyLabel);

        JPanel readyQueue = new JPanel();
        readyQueue.setBounds(30, 100, 150, 200);
        readyQueue.setBackground(Color.LIGHT_GRAY);
        readyQueue.setLayout(new FlowLayout());
        add(readyQueue);

        // CPU BOX
        JLabel cpuLabel = new JLabel("CPU");
        cpuLabel.setBounds(350, 60, 50, 30);
        add(cpuLabel);

        JPanel cpuBox = new JPanel();
        cpuBox.setBounds(300, 100, 150, 150);
        cpuBox.setBackground(Color.CYAN);
        add(cpuBox);

        // Completed Queue
        JLabel completeLabel = new JLabel("Completed:");
        completeLabel.setBounds(600, 60, 100, 30);
        add(completeLabel);

        JPanel completedQueue = new JPanel();
        completedQueue.setBounds(600, 100, 150, 200);
        completedQueue.setBackground(Color.LIGHT_GRAY);
        completedQueue.setLayout(new FlowLayout());
        add(completedQueue);

        // Start Button
        JButton startBtn = new JButton("Start Simulation");
        startBtn.setBounds(320, 300, 150, 30);
        add(startBtn);
